...
use memchr;
use std::{cmp, fmt, str::CharIndices, usize};
...
unsafe impl<'a> Searcher<'a> for CharSearcher<'a> {
    ...
    fn next(&mut self) -> SearchStep {
        ...
        let old_len = iter.size_hint().1.unwrap();
        if let Some(ch) = iter.next() {
            // add byte offset of current character
            // without re-encoding as utf-8
            self.finger += old_len - iter.size_hint().1.unwrap();
            if ch == self.needle {
...
unsafe impl<'a> ReverseSearcher<'a> for CharSearcher<'a> {
    ...
    fn next_back(&mut self) -> SearchStep {
        ...
        if let Some(ch) = iter.next_back() {
            ...
            // without re-encoding as utf-8
            self.finger_back -= old_len - iter.size_hint().1.unwrap();
            if ch == self.needle {
...
unsafe impl<'a, C: MultiCharEq> Searcher<'a> for MultiCharEqSearcher<'a, C> {
    ...
    fn next(&mut self) -> SearchStep {
        ...
        let pre_len = s.size_hint().1.unwrap();
        if let Some((i, c)) = s.next() {
            let len = s.size_hint().1.unwrap();
